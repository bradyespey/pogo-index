...
$(document).ready(function () {
    ...
    $('#resetFiltersButton').on('click', function () {
        if (window.formsTable) {
            // Clear DataTables filters
            window.formsTable.search('').columns().search('').draw();

            // Reset cloned header inputs and selects
            const clonedHeader = window.formsTable.table().header();
            ...
            $('#showEntries').val('10');
            window.formsTable.page.len(10).draw();
        }
    });
    ...
    $('#showEntries').on('change', function () {
        ...
        window.formsTable.page.len(entries).draw();
    });
});
